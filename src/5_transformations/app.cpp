...
void processInput(GLFWwindow* window, float& angle, float& radius);
...
int main()
{
    ...
    // ------------------------------------
    const std::string vs{ shadersDirPath + "/5_transformations/6.2.coordinate_systems.vs" };
    const std::string fs{ shadersDirPath + "/5_transformations/6.2.coordinate_systems.fs" };
    ...
    unsigned char* data = stbi_load(std::string(texturesDirPath + "/5_transformations/container.jpg").c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(std::string(texturesDirPath + "/5_transformations/awesomeface.png").c_str(), &width, &height, &nrChannels, 0);
    if (data)
    {
...
// ---------------------------------------------------------------------------------------------------------
void processInput(GLFWwindow* window, float& angle, float& fov)
{
    constexpr float speed = 7.0f;
    static double formerTime;
    double currentTime = glfwGetTime();
    float delta = currentTime - formerTime;
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS && fov >= 0.0f)
        fov -= speed * 3 * delta, 0.0f;

    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && fov <= 90.0f)
        fov += speed * 3 * delta;
